...
const GENERATED_RUNTIME_OPS: &str = "\
...
fn generate_runtime(check: bool) -> io::Result<()> {
    let output = workspace_root().join("runtime/src/generated/ops.ts");
    if check {
        let current = fs::read_to_string(&output)?;
        if current != GENERATED_RUNTIME_OPS {
    ...
    write_generated_file(&output, GENERATED_RUNTIME_OPS)
}
