...
impl Span {
    pub(crate) const fn from_validated_offsets(source: SourceId, start: usize, end: usize) -> Self {
...
#[cfg(test)]
mod tests {
    use crate::{SourceFile, SourceId, SpanError};
    ...
    fn validates_empty_eof_and_rejects_invalid_ranges() {
        ...
        assert!(source.span(0, 3).is_err());
        assert!(source.span(0, 1).is_err());
    }

    #[test]
    fn merges_only_spans_from_the_same_source() {
        let first = SourceFile::new(SourceId::new(1), "first.rb", "value");
        let second = SourceFile::new(SourceId::new(2), "second.rb", "x");
        let left = first.span(2, 4).unwrap();
        let right = first.span(0, 3).unwrap();
        assert_eq!(left.merge(right).unwrap().range(), 0..4);
        let other = second.span(0, 1).unwrap();
